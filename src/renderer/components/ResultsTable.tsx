...
const ResultsTable: React.FC<ResultsTableProps> = ({
  ...
  const isCellSaving = (recordId: string, column: string): boolean => {
    const cellKey = `${recordId}-${column}`;
    const status = cellStatuses.get(cellKey);
            ...
            {sortedResults?.map((record, index) => (
              ...
              >
                ...
                {columns.map((column) => {
                  ...
                  const isSaving = isCellSaving(record.Id, column);
                    ...
                    >
                      ...
                      {isSaving && !isEditing && (
                        <div className="absolute inset-0 flex items-center justify-center bg-discord-accent/10">
                          <svg className="animate-spin h-4 w-4 text-discord-accent" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
